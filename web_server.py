...
# SSH client for training device
ssh_client = None
training_device_ip = None
...
if ON_RPI:
    ...
    active_pins = {}
    lock = threading.Lock()
    print("GPIO functionality disabled (not on Raspberry Pi)")


def activate_pin(pin_num, duration):
    """Activate a pin for specified duration"""
    if not ON_RPI:
        print(f"Mock: Activating pin {pin_num} for {duration} seconds")
        with lock:
...
@app.route('/api/training/connect', methods=['POST'])
def connect_training_device():
    """Connect to a remote device via SSH for training"""
    global ssh_client, training_device_ip
    data = request.json or {}
    ip = data.get('ip')
    ...
    try:
        ...
        # Assuming default credentials; in production, use key-based auth
        ssh_client.connect(ip, username='pi', password='raspberry', timeout=10)
        training_device_ip = ip
...
def start_training():
    """Start chatbot training on the connected device"""
    global ssh_client
    if not ssh_client:
    ...
    transformer = data.get('transformer', 'gpt2')
    
    # First, install dependencies on remote device
    install_commands = [
    ...
    try:
        ...
        # Simple training script to run on remote device
        training_script = f"""
import torch
from transformers import GPT2LMHeadModel, GPT2Tokenizer, Trainer, TrainingArguments
from datasets import load_dataset

print("Starting training with {epochs} epochs on {dataset} using {transformer}")
...
try:
    if '{dataset}' == 'wikitext':
        ...
        print("Custom dataset not implemented")
        exit(1)
except Exception as e:
...
"""
        ...
        # Run the training script on the remote device in background
        stdin, stdout, stderr = ssh_client.exec_command(f'python -c "{training_script}" > training.log 2>&1 &')
        # Don't wait, return immediately
        return jsonify({'success': True, 'message': 'Training started on remote device. Check training.log for progress.'})
    except Exception as e:
        return jsonify({'error': str(e)}), 400
