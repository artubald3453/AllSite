...
- 🤖 **AI Assistant** - Chat with your robot using natural language
- 🏃 **Remote Training** - Train chatbot models on separate compute devices

---

## Remote Training Feature

The web GUI includes a **Training** tab that allows you to connect to a remote compute device via SSH and train chatbot models.

### Setup

1. Ensure the remote device has SSH enabled and you have credentials
2. The training feature will automatically install required dependencies (PyTorch, Transformers, etc.) on the remote device
3. Connect to the device using its IP address in the Training tab

### Training Options

- **Epochs**: Number of training epochs (default: 10)
- **Dataset**: Choose from WikiText or OpenWebText datasets
- **Transformer**: Select GPT-2 model size (small, medium, large)

### Usage

1. In the web GUI, click the **Training** tab
2. Enter the IP address of your compute device
3. Click **Connect** to establish SSH connection
4. Select your training parameters
5. Click **Start Training** to begin

Training runs asynchronously on the remote device. Check `training.log` on the remote device for progress.
...
- Training
  - POST `/api/training/connect` { `"ip": "192.168.1.100"` } (connect to remote device)
  - POST `/api/training/start` { `"epochs": 10, "dataset": "wikitext", "transformer": "gpt2"` } (start training)
